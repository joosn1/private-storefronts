...
export const loader = async ({ request, params }) => {
  ...
  const isIndex = normalizedPath === `/s/${slug}`;

  if (!isIndex) {
    // Child route (auth/login): return just the branding data
    return {
      storefront: {
        id: storefront.id,
  ...
  // Build custom price lookup: variantId → price string
  ...
  // Build allowed variant set (only variants we have in the DB)
  const allowedVariantIds = new Set(storefrontProducts.map((p) => p.shopifyVariantId));

  // Filter products: skip deleted products, filter to only allowed variants
  const products = shopifyProducts
    ...
    .filter((p) => p.variants.length > 0); // skip products with no allowed variants
...
export default function StorefrontRoute() {
  ...
  // If there's a child route (auth/login), render it wrapped in branding
  if (outlet) {
    return (
...
// ─── Cart Script (client-side Storefront API) ─────────────────────────────────
